...
本集群计算环境基于 K8S 搭建而成，硬件包括3台独立的 master 节点、14台 worker 节点和一部分GPFS存储集群划分出来的存储空间。使用 Harbor 搭建私有镜像仓库，openLDAP 进行统一身份认证。通过统一的 kubeconfig 配置文件分发平台，用户也可以通过 [kubectl](https://kubernetes.io/docs/tasks/tools/install-kubectl-linux/) 以命令行的方式使用K8S集群。


| 系统         | 登陆地址                                 | 功能                 |
| ---------- | ------------------------------------ | ------------------ |
| 密码管理       | https://auth.aixiongan.org.cn:9443   | 账号修改密码。            |
| Harbor     | https://harbor.aixiongan.org.cn:9443 | 私有容器镜像仓库           |
| kubeconfig | https://k8s.aixiongan.org.cn:9443    | kubeconfig配置文件分发平台 |

## 获取访问权限

在管理员已经为用户创建好账号的情况下， 用户需要确认是否已经满足下列三个条件

- 测试网络连接，测试方法为访问 harbor.aixiongan.org.cn:9443， 如果无法访问，检查你的网络设置（特别是DNS设置），或者联络管理员。
...
用浏览器访问URL地址：https://login.aixiongan.org.cn:9443 ，会进入Login界面：

![](assets/dex_login.png)

输入邮箱地址（邮箱地址等同申请的账号，格式为：用户名@aixiongan.org.cn）和密码（上边修改过的密码）即可登录。

>注：这里的邮箱地址是***用户名@aixiongan.org.cn***。
...
kubectl get pvc
```
应该能看到返回了1个或2个PVC （是用户在集群中可以访问的存储空间，可以理解为是一个盘）。
...
ContainerImage: harbor.aixiongan.org.cn:9443/xuw/pytorch:v1.5   # 镜像名称，默认为 harbor.aixiongan.org.cn:9443/xuw/pytorch:v1.5，或者见README的说明
...
在默认的模板中，自动为每个pod默认挂载了用户所需的存储卷。这些存储卷是管理员为用户创建好了用于长期保存数据的[持久卷申领（PersistentVolumeClaim，PVC）](https://kubernetes.io/zh-cn/docs/concepts/storage/persistent-volumes/)。

- 挂载于容器内`/root`路径的GPFS存储集群的PVC，用于存储文档及代码等小文件（个人独占的存储空间、永久存储）
- 挂载于容器内`/scratch`路径的宿主机本地的NVME硬盘（临时存储、容器停止后丢失）
- 挂载于容器内`/nvme-share`路径的NVME盘GPFS集群的共享数据（共享存储、管理员可写、普通用户只读）
- 挂载于容器内`/hdd-share`路径的机械盘GPFS集群的共享数据（共享存储、管理员可写、普通用户只读）

临时数据存放在宿主机本地的NVME硬盘中，挂载在容器内的`/scratch`，PVC被删除后里面的数据也会被删除，请一定不要将需要持久化保存的重要数据放在这个路径。
...
用浏览器访问URL地址 `https://auth.aixiongan.org.cn:9443` 访问密码修改界面。界面如下图：
...
我们从`harbor.aixiongan.org.cn:9443/library/`下的镜像出发，安装`requirements.txt`中的依赖，并安装数据。我们这里不赘述[ Dockerfile 的语法](https://docs.docker.com/engine/reference/builder/)。实例的 Dockerfile 如下：

```docker
# Dockerfile
FROM harbor.aixiongan.org.cn:9443/library/ubuntu-pytorch:1.5.0
COPY . build
...
访问[https://harbor.aixiongan.org.cn:9443](https://harbor.aixiongan.org.cn:9443)，注意这里必须是https，用户名及密码等同用户访问k8s集群的用户名及密码。

> **_NOTE:_** 注意这里的用户名格式为“用户名@aixiongan.org.cn”。
...
假设我们的项目名为 zhangsan，则我们之后的镜像均要 push 到`harbor.aixiongan.org.cn:9443/zhangsan/`下，首先 tag 我们做好的镜像：

```bash
docker tag sample:v0 harbor.aixiongan.org.cn:9443/zhangsan/sample:v0
```

之后将镜像 push 到 Harbor 中，我们需要先在 docker 中登录我们在 Harbor上的账号：

```txt
$ docker logout harbor.aixiongan.org.cn:9443
Removing login credentials for harbor.aixiongan.org.cn:9443
$ docker login harbor.aixiongan.org.cn:9443
Username: zhangsan@aixiongan.org.cn
Password:
Login Succeeded
```
...
```bash
docker push harbor.aixiongan.org.cn:9443/zhangsan/sample:v0
```
